...
<body>
    ...
    <center><h2 style="margin-top:15px;">Update User</h2></center>
    <div class="border-bottom my-3"></div>
        <div id="HDpro">
            <?php
            global $query;
            $user = $query->getUserById($_GET['id']); // get avatar
            if($run->num_rows > 0){
                echo "<img src=../".$user['avatar']." width='200' height='200' style='border-radius:20px; 
                object-fit: cover;'>";
            }
			...
			if(isset($_POST['submit_update'])){
                if(checkValue()){
					...
					$favorite= substr($favorite,0, -1);
						$run = $query->adminUpdateUser($_POST['username'],$_POST['fullname'],
                        $_POST['email'],$_POST['gender'],$favorite, $_POST['role'], $_GET['id']);
						if($run != null)
							echo '<script>alert("Cập nhật thông tin user thành công")
                            ;window.location.href="/zerotype/user/user.php";</script>';
                        else
                            echo '<script>alert("Cập nhật thông tin user thất bại")
                            ;window.location.href="/zerotype/user/user.php";</script>';
                }
            }
			?>
